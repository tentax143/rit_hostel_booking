...
def male_current_bookings_view(request):
    ...
    room_no= Booking.objects.filter(admissionNo=new_admissionNo).values('room_number',"warden_approved",'gm_approved',"student_name").first()
    ...
    print("++++++++++++",room_no)
    button_view = True if room_no['warden_approved'] and room_no["gm_approved"] else False
    return render(request, 'male_current_bookings.html', {'bookings': bookings,'button_view':button_view,'name':room_no['student_name']})
...
def generate_pdf(request):
    ...
    if role == 'staff':
        ...
        if personal_data:
            ...
            print("................................",new_admissionNo.split('/')[0][-5::],admissionNo)
            pdf_url = f'http://localhost:8080/fpdf/hostel_form.php?admissionNo={admissionNo}&new_admissionNo={new_admissionNo}'
            room_no = Booking.objects.filter(admissionNo=new_admissionNo).values("room_number").first()
            Academic_year = new_admissionNo.split('/')[0][-5::]
            subject_create = f'RIT Hostel Room for the Academic Year {Academic_year}'
            message_create = f'''Room No: {room_no['room_number']}  
            ...
            print(message_create,subject_create)
